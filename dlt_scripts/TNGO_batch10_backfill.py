# TNGO backfill script for BATCH 10
...
DB_TABLE = "raw_tngo_batch10_prices_backfill"
S3_BUCKET = os.getenv("S3_BUCKET")
S3_PREFIX = "smp500_ingestion/tngo_batch10_prices_backfill"
...
def upload_csv_to_s3(csv_body, chunk_number):
    s3 = boto3.client("s3")
    file_name = f"tngo_batch10_backfill_chunk{chunk_number:03d}_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.csv"
...
def main():
    ...
    try:
        # Create table if it doesn't exist (batch 10 backfill table is new)
