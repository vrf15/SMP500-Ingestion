# FMP backfill script for HC COMPANIES
...
TICKER_FILE = Path("/opt/airflow/config/smp500_ingestion/FMP_HC_ticker.txt")
...
DB_TABLE = "raw_fmp_hc_prices_backfill"
S3_BUCKET = os.getenv("S3_BUCKET")
S3_PREFIX = "smp500_ingestion/fmp_hc_prices_backfill"

# CSV column order — matches the ingestion script schema
CSV_COLUMNS = [
    "symbol", "date", "open", "high", "low", "close", "volume",
    "change", "changePercent", "vwap",
    "requested_symbol", "source_api", "backfill_range", "ingested_at", "s3_key",
]
...
# Create the table if it doesn't exist — quoted columns for camelCase (changePercent)
...
# Fetching data with 429 retry logic — FMP supports date ranges natively
...
def upload_csv_to_s3(csv_body, chunk_number):
    s3 = boto3.client("s3")
    file_name = f"fmp_hc_backfill_chunk{chunk_number:03d}_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.csv"
...
# Uploading to RDBMS — chunked to avoid memory spikes, quoted column names
