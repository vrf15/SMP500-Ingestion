# FMP ingestion script for IT COMPANY PROFILES
...
DB_TABLE = "raw_fmp_it_profile_daily"
S3_BUCKET = os.getenv("S3_BUCKET")
S3_PREFIX = "smp500_ingestion/fmp_it_profile_daily"
...
def upload_df_to_s3(df):
    s3 = boto3.client("s3")
    file_name = f"fmp_it_profile_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.csv"
