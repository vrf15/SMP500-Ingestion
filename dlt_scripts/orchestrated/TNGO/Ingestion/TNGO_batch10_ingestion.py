# TNGO ingestion script for BATCH 10
...
TICKER_FILE = Path("/opt/airflow/config/smp500_ingestion/TNGO_SP500_BATCH10_ticker.txt")
...
DB_TABLE = "raw_tngo_batch10_prices_daily"
S3_BUCKET = os.getenv("S3_BUCKET")
S3_PREFIX = "smp500_ingestion/tngo_batch10_prices_daily"
...
# Create the table if it doesn't exist — aligned with backfill pattern
...
def upload_csv_to_s3(csv_body):
    s3 = boto3.client("s3")
    file_name = f"tngo_batch10_prices_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.csv"
...
# Uploading to RDBMS — engine passed in from main(), quoted column names for camelCase!
...
def main():
    ...
    try:
        ...
        # Guard: skip upload if no data returned; we prevent tables with empty schema
        if row_count == 0:
