...
# Create the table if it doesn't exist — aligned with TNGO pattern
...
# Fetching data with retry logic... free cannot use start/end date (API CONFIG!)
...
# Uploading to RDBMS — engine passed in from main(), quoted column names
...
def main():
    ...
    try:
        ...
        # Guard: skip upload if no data returned ; prevents empty tables from being returned!
        if row_count == 0:
